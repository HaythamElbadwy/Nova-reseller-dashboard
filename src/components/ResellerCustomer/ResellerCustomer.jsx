...
export default function ResellerCustomer() {
  ...
  const getResellerCustomer = async (page) => {
    ...
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/getSubscriptions?page=${page}`, {
        method: 'GET',
        headers: {
  ...
  const addNewSubscription = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/newSubscription`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const getSearch = async (macAddress) => {
    ...
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/searchByMac?macAddress=${macAddress}&type=reseller`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
            ...
            <div className="relative p-4 w-full max-w-md max-h-full">
              <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div className="p-4 md:p-5">
                  ...
                  <button type="submit"
                    onClick={handleAdd}
                    className="text-white mr-5 inline-flex items-center bg-[#FF9D03] hover:bg-b[#FF9D03] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-800 dark:focus:ring-blue-800">
                    {isLoading ?
