...
export default function Provider() {
  ...
  const getProvider = async (page) => {
    ...
    try {
      const response = await fetch(`https://novatv.onrender.com/reseller/getByCode?page=${page}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const addGenerateCode = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://novatv.onrender.com/reseller/generateCode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const getSearch = async (macAddress) => {
    ...
    try {
      const response = await fetch(`https://novatv.onrender.com/reseller/searchByMac?macAddress=${macAddress}&type=provider`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
