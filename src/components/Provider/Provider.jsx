...
export default function Provider() {
  ...
  const getProvider = async (page) => {
    ...
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/getByCode?page=${page}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const addGenerateCode = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/generateCode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
  ...
  const getSearch = async (macAddress) => {
    ...
    try {
      const response = await fetch(`https://nova-psi-five.vercel.app/reseller/searchByMac?macAddress=${macAddress}&type=provider`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `novON0${localStorage.getItem('authToken')}`
        ...
        <div className={`${styles.provider_options} mt-12 flex items-center justify-around`}>
          ...
          <button type="button"
           ...
           border-[#FF9D03] hover:bg-[#FF9D03] focus:ring-4 focus:outline-none
            focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:border-[#FF9D03] dark:text-[#FF9D03] dark:hover:text-white
             dark:hover:bg-[#FF9D03] dark:focus:ring-[#FF9D03]">
